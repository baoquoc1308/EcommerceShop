...
return (
  <div className="grid">
    <div className={`flex flex-col ${props.mode === 'dark' ? "bg-gray-500 text-white" : "bg-gray-200 text-black"} shadow-xl align-middle my-36 md:w-80 md:mx-auto rounded-xl mx-10`}>
      <h2 className='text-center mt-2'>{display === 'hidden' ? "Not Registered?" : "Already have an account?"}<span className={`${props.mode === 'dark' ? "text-blue-300" : "text-blue-600 cursor-pointer"}`} onClick={register}>Click here</span></h2>
      <h1 className='text-2xl my-4 text-center font-semibold'>Sign in</h1>
      <div className='mx-4'>
        <h3 className='my-2'>Email address</h3>
        <input type="text" placeholder='Enter email' className='my-2  rounded-md w-full p-2 text-black' onChange={(e) => setEmail(e.target.value)} />
        <h3 className='my-2'>Password</h3>
        <input type="password" placeholder='Enter password' className='my-2 rounded-md w-full p-2 text-black' onChange={(e) => setPassword(e.target.value)} />
        {/* for registering */}
        ...
        <button className='my-3 text-center bg-blue-600 w-full rounded-lg text-white p-2' onClick={handleLogin}>Submit</button>
